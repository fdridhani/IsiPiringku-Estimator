...
package id.ac.polkesjakdu.isipiringku.estimator_proporsi.imagesegmentation
...
import id.ac.polkesjakdu.isipiringku.estimator_proporsi.imagesegmentation.camera.CameraFragment
...
class MainActivity : AppCompatActivity(), CameraFragment.OnCaptureFinished {
  ...
  private fun setChipsToLogView(itemsFound: Map<String, Int>) {
    ...
    val labelsFoundTextView: TextView = findViewById(R.id.is_labels_found)
    if (chipsGroup.childCount == 0) {
      labelsFoundTextView.text = getString(R.string.is_no_labels_found)
    } else {
      labelsFoundTextView.text = getString(R.string.is_labels_found)
    }
